...
(() => {
  ...
  const populateServices = () => {
    const list = state.overlay?.querySelector(".swdm-intro__service-list");
    if (!list) return;

    const fragment = document.createDocumentFragment();
    SWDM_INTRO_CONFIG.services.forEach((service, index) => {
  ...
  const activateIntro = (overlay) => {
    ...
    const skipButton = state.overlay.querySelector(".swdm-intro__skip");
    skipButton?.addEventListener("click", endIntro, { once: true });
